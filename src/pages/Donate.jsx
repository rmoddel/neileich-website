...
export default function Donate() {
  ...
  return <div className="donate-page"><section className="donate-hero"><div className="container"><p>SUPPORT NEILEICH</p><h1>Give any amount.</h1><span>Every gift helps children and families thrive.</span></div></section><main className="container donate-shell"><form className="donate-card" onSubmit={submit}><h2>Make a secure donation</h2><p>Choose the amount that is meaningful to you.</p><label>Donation amount<input required name="amount" value={form.amount} onChange={change} type="number" min="1" max="1000000" step="0.01" inputMode="decimal" placeholder="0.00" /></label><div className="donate-grid"><label>Full name<input required name="donorName" value={form.donorName} onChange={change} autoComplete="name" /></label><label>Email<input required type="email" name="donorEmail" value={form.donorEmail} onChange={change} autoComplete="email" /></label></div><label>Phone <em>(optional)</em><input name="donorPhone" value={form.donorPhone} onChange={change} autoComplete="tel" /></label><section className="donate-payment"><h3>Secure card payment</h3><p>Your card details are securely handled by Sola Payments.</p><label>Card number<iframe title="Secure card number" data-ifields-id="card-number" data-ifields-placeholder="Card number" src="https://cdn.cardknox.com/ifields/2.15.2309.2601/ifield.htm" /></label><div className="donate-grid"><label>Expiration (MMYY)<input required value={expiry} onChange={(event) => setExpiry(event.target.value.replace(/\D/g, '').slice(0, 4))} inputMode="numeric" autoComplete="cc-exp" placeholder="MMYY" /></label><label>CVV<iframe title="Secure card CVV" data-ifields-id="cvv" data-ifields-placeholder="CVV" src="https://cdn.cardknox.com/ifields/2.15.2309.2601/ifield.htm" /></label></div><div ref={tokenInputs}><input name="xCardNum" data-ifields-id="card-number-token" type="hidden" /><input name="xCVV" data-ifields-id="cvv-token" type="hidden" /></div></section>{error && <p className="donate-error">{error}</p>}<button className="donate-button" disabled={paying}>{paying ? 'Confirming your donation…' : form.amount ? `Donate $${form.amount}` : 'Donate securely'}</button><small>Your card information is never stored by Neileich.</small></form><section id="donor-advised-fund" className="donor-advised-fund" aria-labelledby="donor-advised-fund-heading"><h2 id="donor-advised-fund-heading">By Donor Advised Fund</h2><p>To make a fee-free gift from your donor-advised fund portal, search for our Tax ID <strong>26-4527675</strong> and select <strong>Neileich</strong>, our legal DBA.</p></section></main></div>
}
