...
create or replace function create_pending_sponsorship(p_type uuid, p_name text, p_email text, p_phone text, p_dedication_type text, p_dedication_text text, p_anonymous boolean, p_date date, p_hebrew_year integer, p_hebrew_month integer, p_hebrew_day integer, p_recurring boolean)
returns table (id uuid, amount_cents integer) language plpgsql as $$
declare v_type sponsorship_types%rowtype; v_count integer; v_id uuid;
begin
  ...
  values (p_type, p_name, lower(p_email), nullif(p_phone,''), p_dedication_type, p_dedication_text, p_anonymous, p_date, p_hebrew_year, p_hebrew_month, p_hebrew_day, v_type.price_cents, p_recurring, now() + interval '20 minutes') returning sponsorships.id into v_id;
