...
1. **Vercel Functions** in `api/` for availability, checkout, Sola webhook, and recurrence processing. This preserves the current host and avoids a separate backend service.
...
3. **Sola iFields + Transaction API** for payment collection; card data never reaches this app. Sola's signed account webhook is the payment-confirmation source of truth.
...
- Sola, Postgres, Resend, and an auth provider require provisioned accounts, production environment variables, and a database migration before payments can be enabled.
...
Browser
  ...
  -> Sola iFields -> Sola Transaction API -> signed Sola webhook -> Postgres + email
...
`DATABASE_URL`, `SOLA_API_KEY`, `SOLA_WEBHOOK_PIN`, `VITE_SOLA_IFIELDS_KEY`,
`RESEND_API_KEY`, `EMAIL_FROM`, `NOTIFICATION_EMAIL`, `APP_URL`, and `CRON_SECRET`.
