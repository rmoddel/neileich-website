...
export async function sendEmail({ to, subject, text, sponsorshipId, donationId, template }) {
  ...
  try {
    ...
    await sql`insert into email_events (sponsorship_id, donation_id, recipient, template, status, provider_message_id, sent_at) values (${sponsorshipId || null}, ${donationId || null}, ${to}, ${template}, 'sent', ${result.data?.id || null}, now())`
  } catch (error) {
    await sql`insert into email_events (sponsorship_id, donation_id, recipient, template, status, error) values (${sponsorshipId || null}, ${donationId || null}, ${to}, ${template}, 'failed', ${String(error.message).slice(0, 500)})`
