...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sql = db()
    const event = await sql`insert into payment_events(provider_event_id, event_type) values (${`sola:${reference}`}, 'sola.sale.approved') on conflict (provider_event_id) do nothing returning id`
    if (!event.length) return res.status(200).json({ received: true })
    const rows = await sql`select s.*, t.name as sponsorship_name from sponsorships s join sponsorship_types t on t.id = s.sponsorship_type_id where s.payment_reference = ${reference}`
    const s = rows[0]
    ...
    await sql`update payment_events set sponsorship_id = ${s.id}::uuid where provider_event_id = ${`sola:${reference}`}`
    if (s.recurring) {
    ...
    await Promise.all([sendEmail({ to: s.donor_email, subject: 'Your Neileich sponsorship is confirmed', text: `Thank you, ${s.donor_name}.\n\nYour sponsorship is confirmed.\n\n${facts}\n\nPayment reference: ${reference}`, sponsorshipId: s.id, template: 'donor_confirmation' }), sendEmail({ to: process.env.NOTIFICATION_EMAIL || 'neileichorg@gmail.com', subject: `New Neileich sponsorship: ${s.sponsorship_name}`, text: `A new paid sponsorship was received.\n\nDonor: ${s.donor_name}\nEmail: ${s.donor_email}\nPhone: ${s.donor_phone || '—'}\n\n${facts}`, sponsorshipId: s.id, template: 'staff_notification' })])
